...
<div class="slideshow">
    <div class="owl-page owl-carousel owl-theme" data-items="screen:0|items:1" data-rewind="1" data-autoplay="1"
        ...
        @foreach ($slider as $v)
            <div class="slideshow-item block aspect-[1366/560]" owl-item-animation>
                <a class="slideshow-image " href="" target="_blank" title="{{ $v['namevi'] }}">
                    <picture>
                        <source media="(max-width: 500px)" srcset="{{ assets_photo('photo','683x351x1',$v['photo'],'thumbs') }}">
                        <img class="w-100" onerror="this.src='assets/images/noimage.png';"
                            src="{{ assets_photo('photo','1366x560x1',$v['photo'],'thumbs') }}" alt="{{ $v['namevi'] }}"
                            title="{{ $v['namevi'] }}" />
                    </picture>
                </a>
            </div>
